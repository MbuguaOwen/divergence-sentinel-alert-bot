...
def test_tv_parity_confirm_mode_triggers():
    eng = StrategyEngine(
        symbol="BTCUSDT",
        timeframe="15m",
        don_len=3,
        ...
        use_bos_confirmation=False,
        max_wait_bars=1,
        use_cvd_gate=False,
    )

    # First divergence -> should trigger on setup confirmation bar
    seq1 = [
        _candle(0, 10, 11, 9, 9, 1),
        _candle(1, 10, 10, 8, 8, 1),
        _candle(2, 10, 11, 9, 9, 1),
        _candle(3, 8, 10, 7, 10, 2),
        _candle(4, 9, 10, 8, 11, 1),  # trigger bar (setup confirm)
    ]
    signals = []
    for c in seq1:
        signals.extend(eng.on_candle(c, allow_signals=True, in_trading_hours=True))
