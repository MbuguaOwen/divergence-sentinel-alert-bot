...
from .indicators import ema_next, sma, pct_change
...
class StrategyEngine:
    ...
    def on_candle(self, c: Candle, *, allow_signals: bool, in_trading_hours: bool) -> List[Signal]:
        """Consume a CLOSED candle. Returns zero or more signals for THIS close."""
        self._append(c)
        signals: List[Signal] = []

        # Need enough bars to confirm a pivot
        if len(self.candles) < (2 * self.pivot_len + 2):
            return signals
